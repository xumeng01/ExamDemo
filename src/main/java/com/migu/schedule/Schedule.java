...
public class Schedule {
    ...
    private Map<Integer,List<Integer>> nameTasks = new HashMap<Integer, List<Integer>>();
    ...
    private void insertSameTask(int taskId){
        ...
        List<Integer> list = nameTasks.get(time);
        if(list==null){
            list = new ArrayList<Integer>();
            nameTasks.put(time,list);
        }
        list.add(taskId);
    }
    ...
    public int scheduleTask(int threshold) {
        ...
        for (Integer time:nameTasks.keySet()){
            List<Integer> list = nameTasks.get(time);
            if(list.size()>1){
